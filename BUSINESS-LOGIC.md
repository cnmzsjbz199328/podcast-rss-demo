# 🎙️ AIPodcast生成系统 - 业务逻辑梳理

## 📋 系统概述

这是一个基于Cloudflare Workers的全自动Podcast生成系统，主要功能是将News内容通过AI转换为Podcast音频，并提供RSS订阅服务。

## 🏗️ 核心业务流程

### 1. Podcast生成流程 (`POST /generate`)

```
用户请求 → Podcast生成器 → News获取 → 脚本生成 → 语音合成 → 文件存储 → 数据库保存 → RSS更新
```

#### 详细步骤：

1. **接收请求**
   - 接受`style`参数（Podcast风格）
   - 默认风格：`news-anchor`

2. **生成剧集ID**
   - 格式：`{style}-{timestamp}-{randomId}`
   - 示例：`news-anchor-2025-11-04T12-30-45-abc123`

3. **News获取与处理**
   - 调用BBC RSS服务获取最新News
   - 过滤和整理News内容
   - 提取标题、描述、链接、发布时间

4. **脚本生成**
   - 使用Google Gemini AI生成Podcast脚本
   - 根据风格应用不同的提示词
   - 返回结构化的Podcast内容

5. **语音合成**
   - 调用IndexTTS API生成音频
   - 支持异步处理和SSE监听
   - 返回音频数据和元信息

6. **文件存储**
   - 将脚本和音频文件存储到R2
   - 生成公开访问URL
   - 返回存储元数据

7. **数据库保存**
   - 将剧集信息保存到D1数据库
   - 更新RSS Feed
   - 返回完整的剧集数据

## 🎯 核心业务对象

### NewsItem（News项）
```javascript
{
  title: string,        // News标题
  description: string,  // News摘要
  link: string,         // 原文链接
  pubDate: Date,        // 发布时间
  guid: string,         // 唯一标识
  category?: string[]   // 分类标签
}
```

### ScriptResult（脚本结果）
```javascript
{
  content: string,      // 生成的脚本内容
  style: string,        // 脚本风格
  wordCount: number,    // 字数统计
  generatedAt: string,  // 生成时间
  metadata: {
    model: string,      // AI模型信息
    usageMetadata: {},  // API使用统计
    styleConfig: {}     // 风格配置
  }
}
```

### VoiceResult（语音结果）
```javascript
{
  audioData: ArrayBuffer, // 音频二进制数据
  format: string,        // 音频格式 ('wav', 'mp3')
  duration: number,      // 时长(秒)
  fileSize: number,      // 文件大小
  style: string,         // 语音风格
  eventId?: string,      // TTS事件ID (异步时)
  isAsync?: boolean,     // 是否异步处理
  metadata: {}           // 语音生成元数据
}
```

### StorageResult（存储结果）
```javascript
{
  scriptUrl: string,    // 脚本文件URL
  scriptKey: string,    // 脚本文件Key
  scriptSize: number,   // 脚本文件大小
  audioUrl?: string,    // 音频文件URL
  audioKey?: string,    // 音频文件Key
  fileSize?: number,    // 音频文件大小
  uploadedAt: string,   // 上传时间
  episodeId: string,    // 剧集ID
  metadata: {
    stats: {},          // 存储统计
    uploadedAt: string  // 上传时间
  }
}
```

### Episode（剧集）
```javascript
{
  id: string,           // 剧集ID
  title: string,        // 剧集标题
  description: string,  // 剧集描述
  audioUrl: string,     // 音频URL
  audioKey: string,     // 音频Key
  scriptUrl: string,    // 脚本URL
  scriptKey: string,    // 脚本Key
  duration: number,     // 时长(秒)
  fileSize: number,     // 文件大小
  style: string,        // Podcast风格
  transcript: string,   // 文字稿
  status: string,       // 状态 ('published', 'draft')
  publishedAt: string,  // 发布时间
  createdAt: string,    // 创建时间
  ttsEventId: string,   // TTS事件ID
  ttsStatus: string,    // TTS状态 ('pending', 'completed', 'failed')
  metadata: {
    newsCount: number,  // News数量
    wordCount: number,  // 字数统计
    generatedAt: string,// 生成时间
    scriptMetadata: {}, // 脚本元数据
    voiceMetadata: {},  // 语音元数据
    storageMetadata: {} // 存储元数据
  }
}
```

## 🔧 核心服务组件

### 1. RSS服务 (BbcRssService)
**职责**: 获取和解析NewsRSS源
- 输入: RSS URL
- 输出: NewsItem[]

### 2. 脚本服务 (GeminiScriptService)
**职责**: 使用AI生成Podcast脚本
- 输入: NewsItem[], style
- 输出: ScriptResult
- 组件:
  - GeminiApiClient: 负责API调用
  - ScriptStyleManager: 风格配置管理
  - ScriptFormatter: 脚本格式化

### 3. 语音服务 (IndexTtsVoiceService)
**职责**: 将脚本转换为音频
- 输入: script(string), style
- 输出: VoiceResult
- 组件:
  - IndexTtsApiClient: API调用和SSE监听
  - IndexTtsAudioProcessor: 音频处理
  - IndexTtsStyleManager: 语音风格配置

### 4. 存储服务 (R2StorageService)
**职责**: 文件存储和管理
- 输入: scriptResult, voiceResult
- 输出: StorageResult
- 组件:
  - R2FileUploader: 文件上传
  - R2FileValidator: 文件验证

### 5. 数据库服务 (D1DatabaseService)
**职责**: 数据持久化
- Episode CRUD操作
- 统计查询

## 📡 API接口设计

### 生成Podcast
```
POST /generate?style={style}
```

**参数:**
- style: Podcast风格 ('news-anchor', 'guo-de-gang', 'emotional')

**响应:**
```json
{
  "success": true,
  "data": {
    "episodeId": "news-anchor-2025-11-04T12-30-45-abc123",
    "title": "Today Hot Topics - 11月4日",
    "description": "Today HotNews：...",
    "newsCount": 5,
    "wordCount": 1200,
    "duration": 180,
    "fileSize": 2880000,
    "scriptUrl": "https://r2.dev/scripts/...",
    "audioUrl": "https://r2.dev/audio/...",
    "eventId": "abc123",
    "isAsync": false,
    "generatedAt": "2025-11-04T12:30:45Z"
  }
}
```

### 获取剧集列表
```
GET /episodes?limit=20&offset=0&style=news-anchor
```

### 获取剧集详情
```
GET /episodes/{episodeId}
```

### RSS Feed
```
GET /rss.xml
```

## 🎨 支持的Podcast风格

### 1. news-anchor (News主播)
- **特点**: 专业、客观、正式
- **适用场景**: News播报、时事分析
- **语音风格**: 标准普通话，语速适中

### 2. guo-de-gang (郭德纲风格)
- **特点**: 幽默、包袱、接地气
- **适用场景**: 娱乐News、生活趣事
- **语音风格**: 带有方言特色，节奏感强

### 3. emotional (情感化风格)
- **特点**: 温暖、共情、富有感染力
- **适用场景**: 感人故事、人文报道
- **语音风格**: 温柔舒缓，情感丰富

## 🔄 异步处理机制

### TTS异步处理流程

1. **请求发送**
   - 调用IndexTTS API
   - 返回event_id表示异步处理

2. **SSE监听**
   - 连接SSE流：`/gradio_api/call/gen_single/{event_id}`
   - 监听事件：`heartbeat`, `complete`, `error`

3. **结果处理**
   - complete: 下载音频文件
   - error: 抛出异常
   - 超时: 返回处理中状态

### 数据库状态管理

- **pending**: TTS正在处理
- **completed**: TTS处理完成
- **failed**: TTS处理失败

## 📊 业务规则

### News处理规则
- 过滤重复News（基于标题相似度）
- 限制News数量（默认5-10条）
- 按时间倒序排列
- 过滤过旧的News（24小时内）

### 脚本生成规则
- 字数控制：1000-2000字
- 包含News标题和关键信息
- 根据风格调整语言风格
- 保持客观中立（除幽默风格外）

### 音频生成规则
- 采样率：22050Hz
- 格式：MP3/WAV
- 时长控制：3-10分钟
- 文件大小限制：50MB

### 存储规则
- 文件命名：`{type}/{style}/{timestamp}-{random}.{ext}`
- 脚本：TXT格式，UTF-8编码
- 音频：MP3格式，高品质
- 缓存策略：1年CDN缓存

## 🚨 错误处理策略

### 网络错误
- 重试机制：3次重试
- 指数退避：1s, 2s, 4s
- 超时控制：30秒

### API错误
- Gemini: 配额、速率限制、服务错误
- IndexTTS: 音频生成失败、SSE错误
- R2/D1: 存储失败、网络错误

### 业务错误
- News获取失败：返回默认News
- 脚本生成失败：返回错误信息
- 音频生成失败：标记为pending，允许重试

## 📈 监控和统计

### 性能指标
- 生成耗时：从请求到完成的总时间
- API调用统计：各服务的成功率和响应时间
- 文件大小统计：脚本和音频的文件大小分布

### 业务指标
- 日生成量：每日成功生成的Podcast数量
- 用户使用情况：API调用频率和模式
- 错误率：各环节的失败率统计

### 系统健康
- 服务可用性：各组件的健康状态
- 存储使用情况：R2和D1的使用量
- 缓存命中率：CDN缓存效果
