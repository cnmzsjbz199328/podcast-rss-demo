...
```
1. 用户请求 → 2. News获取 → 3. 脚本生成 → 4. 语音合成 → 5. 文件存储 → 6. 数据保存 → 7. RSS更新
```
...
### 1. Podcast生成 (Podcast Generation)
- **入口**: `POST /generate?style={style}`
- **流程**: News→脚本→语音→存储→数据库
- **输出**: 完整的Podcast剧集数据
...
| **News源** | BBC RSS Feed | News获取，XML解析 |
| **缓存** | CDN Edge | 静态资源缓存 |
...
| 风格 | 代码 | 特点 | 适用场景 |
|-----|-----|-----|---------|
| News主播 | `news-anchor` | 专业、客观、正式 | News播报、时事分析 |
| 郭德纲风格 | `guo-de-gang` | 幽默、包袱、接地气 | 娱乐News、生活趣事 |
